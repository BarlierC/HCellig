...
scrb_norm<-function(data_counts,log_transform = T,bg.model.pars=NULL,data.type=c("Ref","query"),file.id="mydata",saveData=T,return=T){
  tictoc::tic("Data normalisation (UMI)")
  print("Normalising data")
  if(data.type == "Ref"){
    data_norm<-DataNorm(data = data_counts,log_transform = log_transform,data.type = data.type,file.id = file.id)
  }else if(data.type == "query"){
    #Keep genes found in query & background
    bg.model.pars <- bg.model.pars[which(rownames(bg.model.pars) %in% dimnames(data_counts)[[1]]),]
    data_counts <- data_counts[dimnames(data_counts)[[1]] %in% rownames(bg.model.pars),]
    data_norm<-DataNorm(data = data_counts,log_transform = log_transform,data.type = data.type,bg.model.pars = bg.model.pars,file.id = file.id)
  }
  tictoc::toc()
  print("saving data")
  if(saveData){
    if(data.type=="Ref"){
    dir_name=paste0("Ref_",file.id)
    }else if (data.type=="query"){
...
cleanData <- function(data_counts,dir.name,file.id){
  ...
  qcCells <- perCellQCMetrics(data_counts_cleaned)
  countsCells <- qcCells$sum
  names(countsCells) <- dimnames(data_counts_cleaned)[[2]]
  low.quality.cells <- isOutlier(countsCells, nmads = 3, log = TRUE)
  low.quality.cells <- names(low.quality.cells[which(low.quality.cells == TRUE)])
  data_counts_cleaned <- data_counts_cleaned[,!dimnames(data_counts_cleaned)[[2]] %in% low.quality.cells]

  print("Removing genes expressed in less than 10 cells")
  nonzero.genes <- data_counts_cleaned > 0
  keep_genes <- Matrix::rowSums(nonzero.genes) >= 10
  data_counts_cleaned <- data_counts_cleaned[dimnames(data_counts_cleaned)[[1]] %in% names(keep_genes),]

  return(data_counts_cleaned)
}
...
GetBackground <- function(data_norm,dir_name,file.id,metadata,sample_n,parallelize,ncores,num.boot.sample,fixseed){
  ...
  genes=dimnames(data_norm)[[1]]
  sf<-as.numeric(qlcMatrix::rowMax(data_norm))
  names(sf) <- genes
  #Save scaling factors
  write.table(sf,file=paste0(dir_name,"/",file.id,"_sf.txt"),col.names = F)
  tictoc::toc()

  #Run bootstrapping of the: (sampling, scaling, thresholds computation) & get the resulting background thresholds
  back.thrs.ecdfs <- RunBootstrap(data_norm = data_norm,file.id = file.id,clusterids_vec = clusterids_vec,metadata = metadata,sample_n = sample_n,parallelize=parallelize,ncores=ncores,num.boot.sample = num.boot.sample, fixseed = fixseed)
...
#' @author Céline Barlier
RunBootstrap <- function(data_norm,file.id,clusterids_vec,metadata,sample_n,parallelize,ncores,num.boot.sample,fixseed){
...
RunBackComputations <- function(norm.data,cell_clusterid_mat,ncells.sample,clusterids_vec,file.id){
  ...
  #Get thresholds
  thrs.back <- GetThresholds(processed.back=processed.back,file.id=file.id)
...
SampleBackground <- function(norm.data,cell_clusterid_mat,ncells.sample,clusterids_vec,file.id){
  sampled.back=Background_sampling(counts_norm_mat = norm.data,cell_clusterid_mat = cell_clusterid_mat,ncells.sample = ncells.sample,clusterids_vec =clusterids_vec)
  return(sampled.back)
}
...
PrepQueryData<-function(query_dat,Ref_data,dir,sfback=NA,file.id="query",ncores,bg.model.pars,normalise=T,scale_data=T,saveData=T,return.data=T){
  ...
  query_dat <- cleanData(data_counts=query_dat,dir.name=paste0(getwd(),"/",dir_name),file.id=file.id)

  ##normalisation
  if(normalise){
	  print("Normalising data (UMI): ScTransform")
	  tictoc::tic("Data Normalisation")
	  query_Normdata=scrb_norm(data_counts = query_dat,file.id = file.id,data.type="query",
	                             saveData = T,return = T,bg.model.pars = bg.model.pars)
	    tictoc::toc(log=T)
  }else{
...
#'
#' @return normalized data
DataNorm<-function(data,log_transform=c(TRUE,FALSE),data.type,bg.model.pars=NULL,file.id=NULL){

  if(data.type == "Ref"){
    dat_norm<-sctransform::vst(umi = data,
                               ...
                               method = "qpoisson")
    saveRDS(object = dat_norm$model_pars,file = paste0("./Ref_",file.id,"/",file.id,"_bg_model_pars.rds"))
  }else if(data.type == "query"){
    dat_norm <- vst.query(umi = data,
